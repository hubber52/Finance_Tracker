...
class CustomManager(models.Manager):
    ...
    def get_total(self, field:str, ruser, amount:str):
        ...
        week_totals = self.get_queryset().filter(user = ruser, **{rateType : "Weekly"})
        month_totals = self.get_queryset().filter(user = ruser, **{rateType : "Monthly"})
        year_totals = self.get_queryset().filter(user = ruser, **{rateType : "Annually"})
        week_sum, month_sum, year_sum = 0, 0, 0

        if week_totals:
            week_sum = week_totals.aggregate(total = models.Sum(amountType))['total']
        if month_totals:
            month_sum = (month_totals.aggregate(total = models.Sum(amountType))['total'] * 12 / 365 * 7)
        if year_totals:
            year_sum = (year_totals.aggregate(total = models.Sum(amountType))['total'] / 365 * 7)
        return {(field+amount) : round((week_sum + month_sum + year_sum),2)}
    ...
    def get_discretionary(self, field:str, ruser):
        ...
        return {field + "_discretionary_total" : round(discretionary_week_total + discretionary_month_total + discretionary_year_total, 2)}
