...
class DebtGetView(APIView):
    ...
    def get(self, request):
        try:
            dbItems = DebtModel.objects.filter(user = self.request.user)
        except:
...
class DebtTotalView(APIView):
    ...
    def get(self, request):
        try:
            total = DebtModel.customObject.get_total("debt", request.user, "_amount")
        except:
            return Response(status=status.HTTP_404_NOT_FOUND)
...
class DebtPaymentView(APIView):
    ...
    def get(self, request):
        try:
            payment = DebtModel.customObject.get_total("debt", request.user, "_payment")
        except:
            return Response(status.HTTP_404_NOT_FOUND)
